...
export default async function MiniAppDetailPage({ params }: { params: Promise<{ miniAppId: string }> }) {
  ...
  const [leads, members] = await Promise.all([getMiniAppLeads(workspaceId, miniAppId), getWorkspaceMembers(workspaceId)]);
  ...
  return (
    <div className="flex flex-col gap-4 py-4 sm:py-6 lg:py-8">
      ...
      <MiniAppDetailShell miniApp={miniApp} initialLeads={leads} members={members} />
    </div>
  );
}
