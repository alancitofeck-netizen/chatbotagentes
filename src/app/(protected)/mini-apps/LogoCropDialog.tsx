...
/** Pick + crop only — same react-easy-crop recipe as
 * profile/sections/AvatarUploadDialog.tsx, but doesn't upload anything
 * itself: the actual Storage write needs the mini app's real `id`, which
 * doesn't exist yet during the wizard's branding step for a brand-new mini
 * app (see updateMiniAppBranding's own comment). Hands the cropped webp
 * Blob back to the caller instead. */
export function LogoCropDialog({ open, onClose, onCropped }: { open: boolean; onClose: () => void; onCropped: (blob: Blob) => void }) {
