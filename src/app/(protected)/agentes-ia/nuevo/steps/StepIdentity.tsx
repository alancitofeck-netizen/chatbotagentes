...
import { AGENT_TYPE_PRESETS, MODULE_OPTIONS, HOW_IT_WORKS, FLOW_STEPS, type WizardState, type AgentTypePreset } from "../wizardConfig";
      ...
      <div className="flex flex-1 flex-col gap-4">
        <Card>
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                {AGENT_TYPE_PRESETS.map((t) => (
                  <button
                    key={t.key}
                    type="button"
                    onClick={() => update({ agentType: t.key as AgentTypePreset })}
                    className={cn(
                      "rounded-lg border p-3 text-left transition-colors",
                      state.agentType === t.key ? "border-accent-500 bg-accent-50" : "border-border-default hover:bg-surface-2",
                    )}
                  >
            ...
            {state.agentType !== "referrals" && (
              <div>
                <p className="mb-2 text-sm font-medium text-foreground">¿Con qué contactos puede trabajar?</p>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {MODULE_OPTIONS.map((m) => (
                    <button
                      key={m.key}
                      type="button"
                      onClick={() => update({ moduleKey: m.key })}
                      className={cn(
                        "rounded-lg border p-3 text-left transition-colors",
      ...
      <div className="flex w-full flex-col gap-3 lg:max-w-xs">
        <Card>
          ...
          <ul className="flex flex-col gap-2">
            {HOW_IT_WORKS[state.agentType].map((line) => (
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-neutral-500">
            {FLOW_STEPS[state.agentType].map((step, i, arr) => (
