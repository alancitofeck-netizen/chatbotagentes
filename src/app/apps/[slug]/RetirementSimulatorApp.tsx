...
import { soften, useCountUp, MiniAppButton, MiniAppProgressBar, DecorativeBackground, AgentBar, StepShell } from "@/components/miniApps/uiPrimitives";
import {
  getDiagnosisSummary,
  getExecutiveSummary,
...
export function RetirementSimulatorApp({ app }: { app: PublicMiniAppView<"simulador_retiro"> }) {
  const { config } = app;
  ...
  return (
    ...
    >
      ...
      <div className={`flex w-full flex-col gap-4 transition-[max-width] duration-300 ${phase === "resultado" ? "max-w-2xl" : "max-w-md"}`}>
        <AgentBar app={app} badgeLabel="Simulador" />
