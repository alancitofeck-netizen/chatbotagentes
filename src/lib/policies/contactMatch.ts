 ...
 * lo crea — mismo patrón que leadSync/runner.ts's processRow. Extraído de
 * actions.ts (que tiene "use server" a nivel de archivo, donde una función
 * con un parámetro no serializable como `supabase` no puede vivir) para que
 * tanto la creación manual/PDF como la importación por Excel (import.ts)
 * la reusen sin duplicar la lógica de dedupe. */
