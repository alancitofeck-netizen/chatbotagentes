...
**Layout**
- **Sidebar**: panel flush a pantalla completa (no un dock flotante con margen), fondo `neutral-950`, `z-[60]`, pegado al borde izquierdo — **actualizado 2026-07-31, revierte la actualización 2026-07-09**: colapsado por defecto (72px, solo íconos + logo + `UserMenu`), expande a 280px al hacer hover/foco (`width 220ms ease`, sin reflow del contenido — el panel es `position: fixed` flotando sobre un placeholder reservado que no anima), referencia explícita Linear/Slack/Notion/Vercel. Expandido muestra: logo + nombre de workspace + rol (`Owner`/`Admin`/`Agente`) + botón de pin/unpin, después ítems agrupados por categoría (headers en mayúsculas). El texto de las etiquetas aparece con fade (`opacity`/`translateX`, 180ms, delay solo al expandir — nunca al colapsar) DESPUÉS de que el ancho ya empezó a animar, nunca junto con el ancho. Ítem activo: fondo `accent-500` sólido, texto/ícono blanco, barra izquierda de 3px, glow sutil (`--elevation-glow-accent`, `globals.css`); hover no-activo: `accent-500` al 12% de opacidad, `radius-lg` (12px). Colapsado, el hover de un ícono muestra un tooltip real (`src/components/ui/Tooltip.tsx`, portal, no `title` nativo). Pin persiste en `localStorage` (`gl-sidebar-pinned`), con un script bloqueante en `src/app/layout.tsx` (mismo patrón que `themeInitScript`) para que no haya flash del ancho equivocado en el primer paint. Mobile (`<768px`) sigue siendo el drawer por hamburguesa de `MobileNav.tsx`, sin hover.
