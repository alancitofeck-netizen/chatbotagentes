...
[worker/whatsapp-connector/](worker/whatsapp-connector/) is a **second, independent Node.js project** in this repo — a standalone, always-on process (whatsapp-web.js/Puppeteer, holds one persistent WhatsApp Web session — a real Chromium instance — per connected workspace member) that does **not** deploy to Vercel and is **not** part of the Next.js app's build (its own `worker` folder is excluded from the root `tsconfig.json` for exactly this reason — the two are separate TypeScript projects with separate dependencies). It exists because Vercel serverless functions cannot hold a long-lived connection, which every other part of this app can assume away.

The worker deliberately never lets `whatsapp-web.js` leak past its own `providers/whatsAppWebJsProvider.ts` — everything else in the worker (`sessionManager.ts`, `controlApi.ts`) depends only on a small `WhatsAppService` interface (`whatsAppService.ts`), so a future swap to a different WhatsApp library only touches that one provider file plus the single line in `index.ts` that instantiates it. See that folder's own `README.md` for its env vars/control API/Dockerfile/session-persistence design.
